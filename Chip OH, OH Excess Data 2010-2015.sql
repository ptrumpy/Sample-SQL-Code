...
Select m.Year, m.Month, g.Owner, Sum(g.TotOnHand) as OHUnits, Sum(g.TotOH$) as OHDollars, Sum(TotOHExcess) as OHExcessUnits, Sum(TotOHExcessDollars) as OHExcessDollars,
 ...
 group by m.Year, m.Month, g.Owner
...
Select m.Year, m.Month, Substring(g.SeasYrCo,4,5) as Owner, Sum(g.TotOnHand) as OHUnits, Sum(g.TotOnHand*CompCost) as OHDollars, Sum(CASE WHEN OnOrder = 0 THEN OverUnits WHEN OverUnits - OnOrder < 0 THEN 0 ELSE (OverUnits - OnOrder) END) OHExcessUnits, 
Sum(CASE WHEN OnOrder = 0 THEN OverUnits WHEN OverUnits - OnOrder < 0 THEN 0 ELSE (OverUnits - OnOrder) END *CompCost) as OHExcessDollars,
 ...
 group by m.Year, m.Month, Substring(SeasYrCo,4,5)

 Drop table #ptTotInv
 select Year, Month, Owner, OHUnits,OHDollars, OHExcessUnits, OHExcessDollars, SKUCount, PackCOunt
 into #ptTotInv
 from #ptMonthlyInv
  Union
 select Year, Month, Owner, OHUnits,OHDollars, OHExcessUnits, OHExcessDollars, SKUCount, PackCOunt
...
select Year, Owner, Avg(OHUnits) as AvgOHUnits, Avg(OHDollars) as AvgOHDollars, Avg(OHExcessUnits) as AvgOHExcessUnits, Avg(OHExcessDollars) as AvgOHExcessDollars, Avg(SKUCount) as AvgSkuCount, Avg(PackCOunt) as AvgPackCount
