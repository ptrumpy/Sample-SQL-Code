...
select distinct top 1000  CONVERT(VARCHAR(10), GETDATE(), 101) as Date, 
...
Case bld.Classification
	...
	,pl.QAHold, pl.NLOWhenGoneFlag, Case When pl.NLOPhoneDate is null then '' else convert(varchar(10),pl.NLOPhoneDate,101) End as NLOPhoneDate, Case when pl.NLSDate is null then '' else convert(varchar(10),pl.NLSDate,101) End as NLSDate, pl.NLARStockBypass,
	 b.BuyerName, IsNull(DMR.DMR,'') as DMR,c5.RecycleReason, c5.RecycleCode, c5.AreaResponsible, g.VendorName,isnull(sub.ReplPack1,'') as ReplacementPack, isnull(sub2.OrdPack,'') as OrderedPack, RpInv.Status_code, rpinv.BldgHostID
...
paclab pl on pl.ShipPackNum = c5.PackNum left join  (Select PackNum, Bldg, Sum(QOH) as QOH from Minvboh where QtyCode <>'IM' group by PackNum, Bldg) m on m.PackNum = c5.PackNum
	...
	left join (select distinct wh_id, item_number, status_code, bldgnum, BldgHostID from rpinv join supplychain_misc.dbo.InventoryMatrix imx on rtrim(rpinv.wh_id) = rtrim(imx.location) and rtrim(rpinv.status_code) = rtrim(imx.statuscode)) RpInv ON pl.ShipPackNum = RpInv.item_Number and m.bldg = rpinv.BldgNum
...
--join InventoryMatrix imx on   rpinv.wh_id = imx.location and rpinv.status_code = imx.StatusCode
where pl.Building <>'074' And pl.building Is Not Null AND (pl.dropshipcode ='' or pl.DropShipCode =' ') and ((Net.NetTotals <= 0 and e.UnshippedQty > 0) or (c5.RecycleReason is not null)) --and pl.shippacknum = '6616481'
order by OrdPre, ComponentNum

select * from #ptPacksRequiringAction where shippacknum = '6616481'
...
Where A.[ShipPackNum]=B.[ShippackNum] and A.InvLoc = B.INVLoc and A.Status_Code = B.Status_Code FOR XML PATH('')),1,1,'') As [Status_Code], 
STUFF((SELECT distinct ', ' + A.[BldgHostID] FROM #ptPacksRequiringAction A
Where A.[ShipPackNum]=B.[ShippackNum] and A.InvLoc = B.INVLoc and A.Status_Code = B.Status_Code and A.bldghostid is not null FOR XML PATH('')),1,1,'') As [BldgHostID]
